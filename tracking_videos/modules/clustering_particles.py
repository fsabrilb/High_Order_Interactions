...
def update_contours(
    ...
    for id_ in df_tracked_2["id"].unique():
        ...
        if mask:
            ...
            df_aux_2["lightest_x"] = df_aux_1["lightest_x"].values[0]
            df_aux_2["lightest_y"] = df_aux_1["lightest_y"].values[0]
...
def clustering_consecutive_frames(
    ...
    # Completion of points (minimum of n_particles points)
    ...
    # Filter far away positions with cluster center
    df_new_tracked["cluster_p"] = kmeans.fit_predict(df_new_tracked[["position_x", "position_y"]].values)  # noqa: 501
    map_column_x = df_tracked_old_frame[["id", "position_x"]].set_index("id").to_dict()["position_x"]  # noqa: 501
    map_column_y = df_tracked_old_frame[["id", "position_y"]].set_index("id").to_dict()["position_y"]  # noqa: 501
    distances_x = df_new_tracked["position_x"] - df_new_tracked["cluster_p"].map(map_column_x)  # noqa: 501
    distances_y = df_new_tracked["position_y"] - df_new_tracked["cluster_p"].map(map_column_y)  # noqa: 501
    mask = ((np.abs(distances_x) <= distance_threshold) & (np.abs(distances_y) <= distance_threshold))  # noqa: 501
    df_new_tracked = df_new_tracked[mask]

    if (df_new_tracked["id"].nunique() < n_particles):  # Assumption: ids_old == n_particles # noqa:501
        df_new_tracked = pd.concat([df_tracked_old_frame, df_new_tracked], ignore_index=True)  # noqa: 501
        df_new_tracked["time"] = time_now

    # Update other columns values
    dicc_positions = {
        "p": "position",
        "w": "weighted",
        "d": "darkest",
    ...
    df_new_tracked["lightest_x"] = df_new_tracked.groupby("id")["lightest_x"].transform("mean")  # noqa: 501
    df_new_tracked["lightest_y"] = df_new_tracked.groupby("id")["lightest_y"].transform("mean")  # noqa: 501

    # Get final tracked particles with the same number of n_particles
    temp_cols = [
        "id_p", "id_w", "id_d", "id_l",
        "cluster_p", "cluster_w", "cluster_d", "cluster_l"
    ]
...
# Clustering particles with same number of tracked particles using K-means algorithm ----  # noqa: 501
def clustering_local_frames(
    reader,
    ...
    saved in general dataframe df_tracked with the following columns:
        - id: Particle ID
        - time: Times (Frames)
        - position_x: Centroid position in X-axis
        - position_y: Centroid position in Y-axis
    ...
    distance_threshold : float
        ...
        Default value is 90
    compare_bounds : bool
        Flag for comparation of contours and edges between consecutive frames.
        Default value is False (increase computation time significantly
    similarity_threshold : float
        Structural similarity lower bound between identical IDs contours
        allowed. Default value is 0.95
    n_particles : int
    ...
    times = df_tracked.sort_values(["time"])["time"].unique()
    final_times = df_tracked[df_tracked["id_count"] != n_particles].sort_values(["time"])["time"].unique()  # noqa: 501
    dt = np.max(np.abs(np.diff(times)))
    df_clustered = df_tracked.copy()

    # Review weights for update positions
    if len(weights_previous_time) != len(final_times):
        weights_previous_time = 0.5 * np.ones_like(final_times)

    # Clustering consecutive frames
    for k, time in enumerate(final_times):
        # print("- Init time: {}".format(time))
        if time > 0:  # Assumption: Initial frame is adjusted manually
            index = np.where(times == final_times[k])[0][0]
            ...
            try:
                ...
                df_local_clustered = df_local[df_local["time"] == times[index]]

            # Update areas (df_local_clustered only has one time)
            if compare_bounds:
                try:
                    # Update areas
                    df_local_clustered = update_contours(
                        reader=reader,
                        df_tracked_1=df_local[df_local["time"] == times[index-1]],  # noqa: 501
                        df_tracked_2=df_local_clustered[df_local_clustered["time"] == times[index]],  # noqa: 501
                        type=type,
                        clip_limit=clip_limit,
                        distance_threshold=distance_threshold,
