...
class Higher_Order_Interactions:
        ...
        if noise_type == "RUA":
            if self.num_dimensions == 2:  # 2D
                ...
                for n_ in range(self.num_particles):
                    self.eta_v[n_] = dt * noise_params[0] * np.array([np.cos(angle[n_]), np.sin(angle[n_])])  # noqa: E501
    ...
    def update_moving(self, particles: Particles):
        ...
        for i in range(self.num_particles):
            if particles.moving_types[i] is True:  # Switch from rest to move
    ...
    def integrator_euler(
        ...
        self.update_moving(particles=particles)
        particles.timers -= dt
...
def estimate_sample(
    ...
    s_particle = np.tile(
        np.repeat(
            np.arange(particles.num_particles) + 1,
    ...
    df = pd.DataFrame(
        {
            ...
            # "mass": np.tile(np.repeat(particles.masses, particles.num_dimensions), n_steps),  # noqa: E501
            # "radius": np.tile(np.repeat(particles.radii, particles.num_dimensions), n_steps),  # noqa: E501
            # "moving_type": np.tile(np.repeat(particles.moving_types, particles.num_dimensions), n_steps),  # noqa: E501
